...
export class MediaService {
  ...
  async uploadMediaProduct(files: Express.Multer.File[], createMediaDto: CreateMediaDto, currentUser: UserResponse): Promise<ApiResponse<MediaProductResponse[]>> {
    try {
  ...
  async getMedia(media_id: number): Promise<ApiResponse<MediaEntity>> {
    try {
      ...
      if (!media) {
        return createResponse(HttpStatus.BAD_REQUEST,'Media không tồn tại', null )
      }
      if (!media) {
  ...
  async getMediasByIds(media_ids: number[]): Promise<ApiResponse<MediaEntity[]>> {
    try {
      const find_media = await Promise.all(media_ids.map(async media_id => {
        const media = await this.mediaRepository.findOne({ where: { id: media_id } });
        if (!media) {
            throw new BadRequestException(`Media_id=${media_id} không tồn tại`);
        }
        return media;
    }));
