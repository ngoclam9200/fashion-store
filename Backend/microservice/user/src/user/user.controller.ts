import { Controller, Get, Post, Body, Patch, Param, Delete, UseGuards, Query, UseFilters, HttpException, HttpStatus } from '@nestjs/common';
...
export class UsersController {
  ...
  async findAllUser(@Query() page_limit:PageLimitDto): Promise<ApiResponse<PaginatedResponse<UserEntity>>> {
  ...
  async getUser( getUserRequest:{id: number} ): Promise<ApiResponse<UserEntity>> { 
    try {
  ...
  async getUsersByIds( getUsersByIdsRequest:{user_ids: number[]} ): Promise<ApiResponse<UserEntity[]>> { 
    try {   
      return await this.usersService.getUsersByIds(getUsersByIdsRequest.user_ids);
    } catch (error) {
      throw new HttpException(error, HttpStatus.BAD_REQUEST);
    }
  }
  ...
  @Post('update')
  async updateProfile(@Body() updateProfileDto : UpdateProfileDto, @CurrentUser() currentUser : UserEntity){
    try {
      return await this.usersService.updateProfile(updateProfileDto, currentUser)
    } catch (error) {
      throw new HttpException(error, HttpStatus.BAD_REQUEST);
    }
  }
  ...
  async signup(@Body() userSignupDto : UserSignupDto): Promise<ApiResponse<UserEntity>> {
    try {
  ...
  async checkValidateToken(@Body() tokenRequest:{token : string , id:number}) {
    try {
