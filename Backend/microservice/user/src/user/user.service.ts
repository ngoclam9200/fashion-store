...
export class UserService {
  ...
  async findAllUser(page: number = 1, limit: number = 10): Promise<ApiResponse<PaginatedResponse<UserEntity>>> {
    try {
      ...
      return createPaginatedResponse(HttpStatus.OK, 'OK', mappedUsers, total_record, page, limit);
    } catch (error) {
  ...
  async getUser(id: number): Promise<ApiResponse<UserEntity>> {
    try {
      const user = await this.usersRepository.findOne({ where: { 'id': id } })
      if (!user) return createResponse(HttpStatus.BAD_REQUEST, 'Người dùng không tồn tại', null)
      return createResponse(HttpStatus.OK, 'OK', mapUserResponse(user))
    } catch (error) {
  ...
  async updateProfile(updateProfileDto: UpdateProfileDto, currentUser: UserEntity) {
    try {
      if (!this.validateUserService.checkRoleAndCurrentUser(currentUser, updateProfileDto.id)) throw new UnauthorizedException('Bạn không có quyền chỉnh sửa')
      const userExist = await this.findUserByEmail(updateProfileDto.email)
      if (userExist && userExist.id != updateProfileDto.id) return createResponse(HttpStatus.BAD_REQUEST, 'Email đã tồn tại', null)
      const user = await this.usersRepository.findOne({ where: { 'id': updateProfileDto.id } })
      if (!user) throw new NotFoundException('Người dùng không tồn tại');
  ...
  async signin(userSigninDto: UserSigninDto): Promise<ApiResponse<UserEntity>> {
    try {
      ...
      if (!userExist) return createResponse(HttpStatus.BAD_REQUEST, 'Sai mật khẩu hoặc tài khoản', null)
      const matchPassword = await compare(userSigninDto.password, userExist.password);
      if (!matchPassword) return createResponse(HttpStatus.BAD_REQUEST, 'Sai mật khẩu hoặc tài khoản', null)
      // if (!userExist.roles.includes(Roles.ADMIN) && userSigninDto.is_admin == BooleanNumberEnum.TRUE) return createResponse(HttpStatus.BAD_REQUEST, 'Bạn không có quyền truy cập', null)
  ...
  async signup(userSignupDto: UserSignupDto): Promise<ApiResponse<UserEntity>> {
    try {
      ...
      if (userExist) return createResponse(HttpStatus.BAD_REQUEST, 'Email đã tồn tại', null)
  ...
  async updateAccessToken(updateTokenDto: UpdateTokenDto) {
    try {
      const user = await this.usersRepository.findOne({ where: { 'id': updateTokenDto.id } })
      if (user) {
  ...
  async checkValidateToken(TokenRequest: { token: string, id: number }) {
    try {
      const user = await this.usersRepository.findOne({ where: { 'id': TokenRequest.id, 'access_token': TokenRequest.token } })
