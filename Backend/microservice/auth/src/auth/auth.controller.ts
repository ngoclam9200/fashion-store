...
export class AuthController {
  ...
  async signin(@Body() body: UserSigninDto): Promise<ApiResponse<UserResponse>>{
    try {
      return  await this.authService.signin(body);
    } catch (error) {
      throw new HttpException('Đã có lỗi xảy ra' + error, HttpStatus.BAD_REQUEST);
    }
  }
  ...
  @PublicAPI()
  @Post('signup')
  async signup(@Body() userSignupDto: UserSignupDto): Promise<ApiResponse<UserResponse>> {
    try {
