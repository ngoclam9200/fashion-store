...
import { UserResponse, mapUserResponse } from 'src/utils/response/user.response';
...
export class AuthService {
  ...
  async signin(userSignInDTO: UserSigninDto): Promise<ApiResponse<UserResponse>> {
    try {
      const response = await lastValueFrom(this.userServiceGrpc.signin(userSignInDTO));
      ...
      if (response.status == HttpStatus.OK) {
        ...
        return createResponse(response.status, response.message, mapUserResponse(response.data, accessToken));
      }
  ...
  async signup(userSignupDto: UserSignupDto): Promise<ApiResponse<UserResponse>> {
    try {
      const response = await lastValueFrom(this.userServiceGrpc.signup(userSignupDto));
      if (response.status == HttpStatus.OK) {
        return response;
      }
      return createResponse(response.status, response.message, null);
  ...
  async updateAccessToken(updateTokenDto: UpdateTokenDto) {
    const response = await lastValueFrom(this.userServiceGrpc.updateAccessToken(updateTokenDto));
  }
}
